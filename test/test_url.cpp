...
namespace powerloader
{
    ...
    TEST_SUITE("url")
    {
        TEST_CASE("parse")
        {
            ...
            {
                URLHandler m("http://just:pass@mamba.org:1233/123/test.path");
                CHECK_EQ(m.scheme(), "http");
                CHECK_EQ(m.path(), "/123/test.path");
                CHECK_EQ(m.host(), "mamba.org");
                CHECK_EQ(m.user(), "just");
                CHECK_EQ(m.password(), "pass");
                CHECK_EQ(m.url_without_path(), "http://just:pass@mamba.org:1233");
            }
            {
                URLHandler m("http://mamba.org");
                CHECK_EQ(m.scheme(), "http");
                CHECK_EQ(m.path(), "/");
                CHECK_EQ(m.host(), "mamba.org");
                CHECK_EQ(m.url_without_path(), "http://mamba.org");
            }
            // {
        ...
        //         std::string input2 =
        //         "https://u:p@conda.io/t/a_-12345-absdj12345-xyxyxyx/more/path"; std::string
        //         remaining_url, scheme, auth, token; split_scheme_auth_token(input, remaining_url,
        //         scheme, auth, token); CHECK_EQ(remaining_url, "conda.io/more/path");
        ...
        //         CHECK_EQ(token, "");
        // #else
        //         split_scheme_auth_token("file:///home/wolfv/test.json", remaining_url, scheme,
        //         auth, token); CHECK_EQ(remaining_url, "/home/wolfv/test.json"); CHECK_EQ(scheme,
        //         "file"); CHECK_EQ(auth, ""); CHECK_EQ(token, "");
        // #endif
        //     }
