...
namespace powerloader
{
    ...
    TEST(url, parse)
    {
        ...
        }
        {
            ...
            EXPECT_EQ(m.scheme(), "http");
            EXPECT_EQ(m.path(), "/123/test.path");
            EXPECT_EQ(m.host(), "mamba.org");
            EXPECT_EQ(m.user(), "just");
            EXPECT_EQ(m.password(), "pass");
            EXPECT_EQ(m.url_without_path(), "http://just:pass@mamba.org:1233");
        }
        {
            URLHandler m("http://mamba.org");
            EXPECT_EQ(m.scheme(), "http");
            EXPECT_EQ(m.path(), "/");
            EXPECT_EQ(m.host(), "mamba.org");
            EXPECT_EQ(m.url_without_path(), "http://mamba.org");
        }
        // {
    ...
    //         EXPECT_EQ(token, "");
    // #else
    //         split_scheme_auth_token("file:///home/wolfv/test.json", remaining_url, scheme, auth,
    //         token); EXPECT_EQ(remaining_url, "/home/wolfv/test.json"); EXPECT_EQ(scheme, "file");
    //         EXPECT_EQ(auth, "");
    //         EXPECT_EQ(token, "");
    // #endif
    //     }
