...
class GrowingFile:
    ...
    def get_percentage(self, delta_size, header_map):
        ...
        map["percentage to download"] = round(100 * float(dsize_list[of_idx[0] - 1]) / float(dsize_list[of_idx[0] + 1]))
        map["percentage matched chunks"] = round(100 * float(dsize_list[of_idx[1] - 1]) / float(dsize_list[of_idx[1] + 1]))
    ...
    def add_content(self):
        ...
        out = subprocess.check_output(["zck", str(self.path), "-o", str(self.path) + ".zck"])
        percentage = -1
        try:
            delta = subprocess.check_output(["zck_delta_size", str(self.path) + ".zck", str(self.path) + "_old.zck"])
