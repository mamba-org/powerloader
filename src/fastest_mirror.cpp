...
bool lr_fastestmirror_perform(std::vector<InternalMirror>& mirrors,
                              std::size_t length_of_measurement);

void fastest_mirror(const std::vector<std::string>& urls)
{
    std::vector<InternalMirror> check_mirrors;
    for (const std::string& u : urls)
    {
        ...
        InternalMirror im{.url = u, .handle = handle, .plain_connect_time = -1};
        check_mirrors.push_back(im);
    }
    lr_fastestmirror_perform(check_mirrors, 1000000);
}
                              ...
                              std::size_t length_of_measurement)
{
    ...
    do {
        ...
        elapsed_micros = std::chrono::duration_cast<std::chrono::microseconds>(tend - tbegin).count();
